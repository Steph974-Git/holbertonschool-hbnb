...
# Modèle pour mise à jour utilisateur normal
user_update_model = api.model('UserUpdate', {
    'first_name': fields.String(required=False, description='First name of the user'),
    'last_name': fields.String(required=False, description='Last name of the user')
})

# Modèle pour création admin
user_admin_model = api.model('UserAdmin', {
    'first_name': fields.String(required=True, description='First name of the user'),
    'last_name': fields.String(required=True, description='Last name of the user'),
    'email': fields.String(required=True, description='Email of the user'),
    'password': fields.String(required=True, description='Password for the user account'),
    'is_admin': fields.Boolean(required=False, description='Admin status (admin only)')
})

# Modèle pour mise à jour admin
user_admin_update_model = api.model('UserAdminUpdate', {
    'first_name': fields.String(required=False, description='First name of the user'),
    'last_name': fields.String(required=False, description='Last name of the user'),
...
class UserList(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            # L'admin peut définir le statut admin (par défaut False)
...
class UserResource(Resource):
    ...
    def put(self, user_id):
        """Update user details (limited for regular users, full access for admins)"""
        current_user = get_jwt_identity()
...
class AdminUserResource(Resource):
    ...
    @jwt_required()
    def put(self, user_id):
        """Update any user's details (Admin only - can modify email and password)"""
        current_user = get_jwt_identity()
        ...
        try:
            ...
            # Validation du password si modifié
            if 'password' in user_data and len(user_data['password']) < 8:
                return {'error': 'Password must be at least 8 characters long'}, 400
